...
if(isset($_POST['submit'])){
  ...
  if (empty($_POST['username']) OR empty($_POST['email']) OR empty($_POST['password']) OR empty($_POST['re-password'])) {
    ...
    //checks if my given password matches with my re-typed password

    if ($password == $repassword){             
           
     if(strlen($email) > 20) {
      echo "<script> alert ('The given email is too big') </script>";

     }  else{


      //checking for existing username and password
